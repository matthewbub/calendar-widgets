...
- `year` - Required. An integer representing the year for which to generate the calendar, between 1900 and 2100.
- `locale` - Optional. A string representing the locale to use when generating the calendar. Defaults to `'en-US'`.

## Return Value

The function returns an object representing a calendar year with the number of days and a list of days for each month.

If the `year` parameter is not a valid year between 1900 and 2100, the function returns an object with an error message.
...
## Errors
The function throws an error if the `year` parameter is not a valid year between 1900 and 2100.
